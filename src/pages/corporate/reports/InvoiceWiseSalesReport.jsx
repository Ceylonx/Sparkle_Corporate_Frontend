...
import { getInvoiceWiseSalesReport } from "../../../services/ReportServices";
import { BeatLoader } from "react-spinners";
import { getAllCustomers } from "../../../services/CustomerServices";
import { getAllItemTypes } from "../../../services/Retail/RetailSettingsServices";

const InvoiceWiseSalesReport = () => {
    ...
    const fetchReportData = async (month, year) => {
        try {
            ...
            const response = await getInvoiceWiseSalesReport(payload);
            const filtered = response.data.data.filter(i => i.items.length > 0);

            const mappedData = filtered.map(({ items, total_amount, discount, is_discount_percentage, created_at, ...rest }) => {
                const totalQuantity = items.reduce(
                    (sum, item) => sum + Number(item.quantity),
                    0
                );

                const totalAmount = is_discount_percentage === 1
                    ? (Number(total_amount) * 100) / (100 - Number(discount))
                    : Number(total_amount) + Number(discount);

                const discountAmount = is_discount_percentage === 1
                    ? totalAmount * Number(discount) / 100
                    : Number(discount);
                ...
                return {
                    ...
                    discount: is_discount_percentage === 1 ? `${Number(discount)}%` : Number(discount).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 }),
                    discount_amount: discountAmount.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 }),
                    created_at: created_at.split("T")[0],
                    total_bill_amount: Number(total_amount).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })
                };
            });
    ...
    const filteredData = data.filter((d) => {
        ...
        return matchesCustomerFilter && matchesDateFilter;
    });
    ...
    const exportCSV = () => {
        ...
        const columnKeyMap = {
            ...
            "Total Bill Amount": data => data.total_bill_amount,
        };
        ...
        link.setAttribute("download", "Invoice_Wise_Sales_Report.csv");
    ...
    return (
        <div className="flex flex-col">
            ...
            <div className="flex flex-row gap-x-3 items-center">
                ...
                <h1 className="text-3xl text-primary font-bold">Invoice Wise Sales Report</h1>
            </div>
            ...
            <div className="flex flex-col rounded-xl bg-white border border-dashed py-5 px-10 gap-y-5 mb-5">
                ...
                <div className="flex flex-col gap-y-3">
                    <div className="flex flex-row items-center gap-x-2">
                        ...
                        <button className="ms-auto hover:text-red-500"
                            onClick={() => {
                            ...
                            {[...Array(10)].map((_, index) => {
                                ...
                                return (
                                    <tr key={index} className={`text-lg py-1.5 px-5 h-10 ${index % 2 === 0 ? "bg-white" : "bg-primary/10"} items-center`}>
                                        ...
                                        {reportColumns[5].state && <td className="px-5 py-2">{data?.quantity}</td>}
                                        {reportColumns[5].state && <td className="px-5 py-2">{data?.total_amount}</td>}
                                        {reportColumns[6].state && <td className="px-5 py-2">{data?.discount}</td>}
                                        {reportColumns[7].state && <td className="px-5 py-2">{data?.discount_amount}</td>}
                                        {reportColumns[8].state && <td className="px-5 py-2">{data?.total_bill_amount}</td>}
                                    </tr>
                                )
...
export default InvoiceWiseSalesReport;