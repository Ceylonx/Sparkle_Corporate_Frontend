...
import { getAllRetailPendingProductions, sendToProductionOrder } from "../../../services/Retail/RetailToProductionServices";
...
import { getTransferNoteTabActions, hasSalesRetailOutletTransferNoteSendAction, parsePermissionTokens } from "../../../utils/retailSubTabPermissions";
import { isSuperadminRole } from "../../../utils/permissionHelper";

const SalesRetailViewToProductionPending = () => {
    ...
    /** Marking/undoing a Return on Outlet Transfer Note items requires this tab's own Edit permission. */
    const canReturnItems = tnActions.outlet.edit;
    ...
    const fetchAllToProductionOrders = async () => {
        try {
            ...
            try {
                ...
                const searchRes = await searchOrderById(userId, String(id), 0, branchId, 50, "Order Entry");
                ...
                console.warn("search-order-by-id failed in to-production pending view:", err);
            }
            ...
            const response = await getAllRetailPendingProductions(userId, branchId);
            let order = response?.data?.pending_production?.find(o => o.order_id === id);

            if (!order) {
                const branchIds = await getBranchIdsFromApi();
                for (const bid of branchIds) {
                    const res = await getAllRetailPendingProductions(userId, bid);
                    const found = res?.data?.pending_production?.find(o => o.order_id === id);
                    if (found) {
                        order = found;
                        break;
                    }
    ...
    const handleSendToProduction = async (serviceTypeId) => {
        if (!canSendOutletTransferNote) return;
        try {
            setIsLoadingSubmit(true);
    ...
    const pendingByService = (serviceTypeId) => {
        ...
        return items.filter((o) => {
            ...
            // For search-order-by-id payloads, flag can be missing; treat missing as pending.
            return sentToProduction == null || Number(sentToProduction) === 0;
        });
    };

    const allPendingItems = productionOrder?.items || [];
    ...
    return (
        <div>
            ...
            {pendingByService(1).length > 0 &&
                <div className="flex flex-col gap-y-3 bg-white rounded-xl p-5 mb-3">
                    <div className="flex flex-row justify-between items-center">
                        <h2 className="text-2xl font-medium">Washing ({pendingByService(1).length} Items)</h2>
                    ...
                    <div className="rounded-xl border border-black/50 overflow-hidden">
                        ...
                        {pendingByService(1).map((order, index) => (
                            <div key={index} className={`grid grid-cols-8 gap-x-3 text-lg py-1.5 px-3 ${index % 2 === 0 ? "bg-white" : "bg-primary/10"} items-center`}>
                                ...
                                <div
                                    ...
                                    onClick={() => {
                                        ...
                                        if (returnedItems.includes(returnId)) { handleUndoReturn(returnId); return; }
                                        if (isSelectMode && returnServiceTypeId === 1) {
            ...
            {pendingByService(2).length > 0 &&
                <div className="flex flex-col gap-y-3 bg-white rounded-xl p-5 mb-3">
                    <div className="flex flex-row justify-between items-center">
                        <h2 className="text-2xl font-medium">Pressing ({pendingByService(2).length} Items)</h2>
                        <div className="flex flex-row gap-x-3">
                            {canReturnItems && (
                                <button
                                    className="cursor-pointer flex flex-row text-primary font-bold text-xl items-center gap-x-2 border border-primary rounded-full px-3 py-2"
                    ...
                    <div className="rounded-xl border border-black/50 overflow-hidden">
                        ...
                        {pendingByService(2).map((order, index) => (
                            <div key={index} className={`grid grid-cols-8 gap-x-3 text-lg py-1.5 px-3 ${index % 2 === 0 ? "bg-white" : "bg-primary/10"} items-center`}>
                                <p>{getItemNameByItemId(order)}</p>
                                ...
                                <div className={`flex flex-col items-center justify-center ${returnedItems.includes(getReturnItemId(order)) ? "text-green-600 cursor-pointer" : "cursor-pointer"}`}
                                    onClick={() => {
                                        ...
                                        if (!returnId || returnLoadingItems.includes(returnId)) return;
                                        if (returnedItems.includes(returnId)) { handleUndoReturn(returnId); return; }
                                        if (isSelectMode && returnServiceTypeId === 2) setSelectedForReturn(prev => prev.includes(returnId) ? prev.filter(id => id !== returnId) : [...prev, returnId]);
                                        else if (!isSelectMode) handleItemReturn(returnId);
                                    }}>
            ...
            {pendingByService(3).length > 0 &&
                <div className="flex flex-col gap-y-3 bg-white rounded-xl p-5 mb-3">
                    <div className="flex flex-row justify-between items-center">
                        <h2 className="text-2xl font-medium">Dry Clean ({pendingByService(3).length} Items)</h2>
                        <div className="flex flex-row gap-x-3">
                            {canReturnItems && (
                                <button
                                    className="cursor-pointer flex flex-row text-primary font-bold text-xl items-center gap-x-2 border border-primary rounded-full px-3 py-2"
                    ...
                    <div className="rounded-xl border border-black/50 overflow-hidden">
                        ...
                        {pendingByService(3).map((order, index) => (
                            <div key={index} className={`grid grid-cols-8 gap-x-3 text-lg py-1.5 px-3 ${index % 2 === 0 ? "bg-white" : "bg-primary/10"} items-center`}>
                                <p>{getItemNameByItemId(order)}</p>
                                <p>{order.color}</p><p>{order.brand}</p><p className="col-span-2">{order.remark}</p><p>{order.packing_option}</p>
                                <p>Rs {Number(getDisplayPrice(order)).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</p>
                                <div className={`flex flex-col items-center justify-center ${returnedItems.includes(getReturnItemId(order)) ? "text-green-600 cursor-pointer" : "cursor-pointer"}`}
                                    onClick={() => {
                                        ...
                                        if (!returnId || returnLoadingItems.includes(returnId)) return;
                                        if (returnedItems.includes(returnId)) { handleUndoReturn(returnId); return; }
                                        if (isSelectMode && returnServiceTypeId === 3) setSelectedForReturn(prev => prev.includes(returnId) ? prev.filter(id => id !== returnId) : [...prev, returnId]);
                                        else if (!isSelectMode) handleItemReturn(returnId);
                                    }}>
                        ...
                        <div className="flex flex-row justify-between items-center">
                            ...
                            <div className="flex flex-row gap-x-3">
                                {/* <button
                                    ...
                                    onClick={() => {
                                        const detectedServiceTypeId = Number(allPendingItems?.[0]?.service_type_id) || 1;
                                        setSelectedServiceTypeId(detectedServiceTypeId);
                                        setShowConfirmationDialog(true);
                                    }}
                                >
                                    <Icon icon={"mynaui:send-solid"} /> Outlet Transfer Note
                                </button> */}
...
export default SalesRetailViewToProductionPending;