...
import { useRetailMenuItems } from "./SalesRetailMenuItems";
import { useEffect, useState } from "react";
import { Icon } from "@iconify/react/dist/iconify.js";
import { Link, useLocation } from "react-router-dom";

const SalesRetailSidebar = () => {
    const menuItems = useRetailMenuItems();
    ...
    return (
        <div className="relative bg-white min-w-44 z-10 flex flex-col h-screen pt-3 pb-5 overflow-hidden">
            ...
            <div className="flex flex-row items-end z-10">
                <img src={logo} className="w-24" />
                <p className="text-primary text-2xl font-bold">SALES</p>
            </div>
                ...
                {filteredMenuItems
                    .map((item, index) => (
                        <Link to={item.page} className={`flex flex-row px-5 items-center py-3 gap-x-2 border-l-3 ${index === selected ? "border-primary bg-gradient-to-r from-primary/60 to-transparent" : "border-transparent"}`} key={index}>
                            ...
                            <p className={`font-bold text-xl ${index === selected ? "text-primary" : "text-black/50"}`}>{item.name}</p>
                        </Link>
                    ))}
            </div>
            ...
            <a href={import.meta.env.VITE_APP_BASE_URL || window.location.origin} className="flex flex-row items-center px-5 mt-auto gap-x-2 text-black/50 hover:text-red-500">
                ...
                <p className="font-bold text-xl">Back to Apps</p>
            </a>
        </div>
    );
};

export default SalesRetailSidebar;