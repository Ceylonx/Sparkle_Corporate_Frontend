...
import { updateRetailDriver } from "../../../services/Retail/RetailDriverServices";
import Swal from "sweetalert2";

const RetailUpdateDriverDialog = ({ driver, handleClose, refreshDrivers }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            setIsLoading(true);
            const res = await updateRetailDriver(driver.id, { name: name.trim(), nic: cleanNic });
            if (res && res.success) {
                Swal.fire({
                    icon: "success",
                    title: "Success",
            ...
            Swal.fire({
                icon: "error",
                title: "Error",
                text: "An error occurred while updating driver"
            });
        } finally {
            setIsLoading(false);
        }
    ...
    return (
        <div className="absolute top-0 left-0 h-screen w-screen z-50 backdrop-blur-sm bg-black/10">
            <div className="relative top-1/2 left-1/2 transform -translate-y-1/2 -translate-x-1/2 bg-canvas rounded-3xl w-1/3 h-fit max-h-[90%] overflow-y-auto flex flex-col p-5">
                <h1 className="text-primary text-3xl font-bold text-center mb-2">Edit Driver Details</h1>
                <p className="text-black/50 text-center mb-5">Modify driver name and NIC number details.</p>
...
export default RetailUpdateDriverDialog;
