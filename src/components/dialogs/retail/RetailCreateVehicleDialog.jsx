...
import { createRetailVehicle } from "../../../services/Retail/RetailVehicleServices";
import Swal from "sweetalert2";

const RetailCreateVehicleDialog = ({ handleClose, refreshVehicles }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            setIsLoading(true);
            const res = await createRetailVehicle({
            ...
            if (res && res.success) {
                ...
                Swal.fire({
                    icon: "error",
                    title: "Error",
                    text: res?.message || "Failed to add vehicle"
                });
            }
        } catch (error) {
            ...
            Swal.fire({
                icon: "error",
                title: "Error",
                text: "An error occurred while creating vehicle"
            });
        } finally {
            setIsLoading(false);
        }
    ...
    return (
        <div className="absolute top-0 left-0 h-screen w-screen z-50 backdrop-blur-sm bg-black/10">
            <div className="relative top-1/2 left-1/2 transform -translate-y-1/2 -translate-x-1/2 bg-canvas rounded-3xl w-1/3 h-fit max-h-[90%] overflow-y-auto flex flex-col p-5">
                <h1 className="text-primary text-3xl font-bold text-center mb-2">Add New Vehicle</h1>
                <p className="text-black/50 text-center mb-5">Save vehicle number details.</p>
...
export default RetailCreateVehicleDialog;
